...
def download_mpiigaze_model() -> pathlib.Path:
    ...
    if not output_path.exists():
        ...
        torch.hub.download_url_to_file(
            'https://github.com/iamsamkhan/pytorch_mpiigaze_demo/releases/download/v0.1.0/mpiigaze_resnet_preact.pth',
...
def download_mpiifacegaze_model() -> pathlib.Path:
    ...
    if not output_path.exists():
        logger.debug('Download the pretrained model')
        torch.hub.download_url_to_file(
            'https://github.com/iamsamkhan/pytorch_mpiigaze_demo/releases/download/v0.1.0/mpiifacegaze_resnet_simple.pth',
            output_path.as_posix())
    else:
        logger.debug(f'The pretrained model {output_path} already exists.')
    return output_path
...
def download_ethxgaze_model() -> pathlib.Path:
    ...
    if not output_path.exists():
        logger.debug('Download the pretrained model')
        torch.hub.download_url_to_file(
            'https://github.com/iamsamkhan/pytorch_mpiigaze_demo/releases/download/v0.2.2/eth-xgaze_resnet18.pth',
            output_path.as_posix())
    else:
        logger.debug(f'The pretrained model {output_path} already exists.')
    return output_path
